...
class AuraModals {
  ...
  showHeaderFooterModal(config = {}) {
    const container = document.getElementById('modal-container');
    const isEn = window.AURA && window.AURA.currentLang === 'en';
    const cfg = window.AURA.activeDocument.pageConfig || {
      startPageNumber: 2,
      numFormat: 'arabic', // 'arabic' | 'roman'
      applyOnlyOdd: false,
      headerText: '',
      footerText: ''
    };
    ...
    container.innerHTML = `
      <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
        <div class="glass-panel w-full max-w-lg rounded-2xl p-5 sm:p-6 border border-slate-700 shadow-2xl flex flex-col gap-5">
          <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 class="text-base font-bold text-white flex items-center gap-2">
              <i data-lucide="layout-template" class="w-5 h-5 text-purple-400"></i> ${isEn ? 'Page Numbering, Header & Footer' : 'Configurar Numeração, Cabeçalho e Rodapé'}
            </h3>
            <button onclick="AURA.closeModal()" class="text-slate-400 hover:text-white"><i data-lucide="x" class="w-5 h-5"></i></button>
          </div>
          ...
          <div class="flex flex-col gap-3 text-xs">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label class="block text-slate-300 font-medium mb-1">${isEn ? 'Numbering Format:' : 'Formato dos Números:'}</label>
                <select id="modal-pg-format" class="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500">
                  <option value="arabic" ${cfg.numFormat === 'arabic' ? 'selected' : ''}>Arábicos (1, 2, 3... — ABNT/APA)</option>
                  <option value="roman" ${cfg.numFormat === 'roman' ? 'selected' : ''}>Romanos minúsculos (i, ii, iii... — Pré-textual)</option>
                  <option value="roman_upper" ${cfg.numFormat === 'roman_upper' ? 'selected' : ''}>Romanos maiúsculos (I, II, III...)</option>
                </select>
              </div>

              <div>
                <label class="block text-slate-300 font-medium mb-1">${isEn ? 'Start displaying from Page:' : 'Exibir número a partir da página:'}</label>
                <input type="number" id="modal-pg-start" value="${cfg.startPageNumber || 2}" min="1" max="50" class="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500">
                <span class="text-[10px] text-slate-400 mt-1 block">${isEn ? 'E.g., Start from 2 to hide cover/title page' : 'Ex: 2 para ocultar número na capa / folha de rosto (ABNT)'}</span>
              </div>
            </div>

            <div>
              <label class="flex items-center gap-2 text-slate-200 cursor-pointer">
                <input type="checkbox" id="modal-pg-odd" ${cfg.applyOnlyOdd ? 'checked' : ''} class="rounded bg-slate-900 border-slate-700 text-purple-500">
                <span>${isEn ? 'Display page numbers only on Odd Pages (Double-sided print)' : 'Exibir numeração apenas em páginas ímpares (Frente e verso)'}</span>
              </label>
            </div>

            <div class="border-t border-slate-800 pt-3">
              <label class="block text-slate-300 font-medium mb-1">${isEn ? 'Header Text (optional):' : 'Texto Personalizado do Cabeçalho (Opcional):'}</label>
              <input type="text" id="modal-pg-header" value="${cfg.headerText || ''}" placeholder="Ex: Universidade Católica de Pernambuco • PPGCC" class="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500">
            </div>

            <div>
              <label class="block text-slate-300 font-medium mb-1">${isEn ? 'Footer Notes / Text (optional):' : 'Texto / Nota do Rodapé (Opcional):'}</label>
              <input type="text" id="modal-pg-footer" value="${cfg.footerText || ''}" placeholder="Ex: Projeto submetido ao Edital FAPESP 2026" class="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500">
            </div>
          </div>

          <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-800">
            <button onclick="AURA.closeModal()" class="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold">${isEn ? 'Cancel' : 'Cancelar'}</button>
            <button onclick="AURA.saveHeaderFooterConfig()" class="px-5 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold shadow-lg shadow-purple-600/30">${isEn ? 'Apply to Document' : 'Aplicar à Página'}</button>
          </div>
        </div>
      </div>
    `;
  ...
  showHistoryModal(historyStack = [], currentIndex = 0) {
    const container = document.getElementById('modal-container');
    const isEn = window.AURA && window.AURA.currentLang === 'en';
    ...
    container.innerHTML = `
      <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
        <div class="glass-panel w-full max-w-lg rounded-2xl p-5 sm:p-6 border border-slate-700 shadow-2xl flex flex-col gap-4">
          <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 class="text-base font-bold text-white flex items-center gap-2">
              <i data-lucide="history" class="w-5 h-5 text-indigo-400"></i> ${isEn ? 'Edit History & Saved Snapshots' : 'Histórico de Edições & Versões Salvas'}
            </h3>
            <button onclick="AURA.closeModal()" class="text-slate-400 hover:text-white"><i data-lucide="x" class="w-5 h-5"></i></button>
          </div>
          ...
          <p class="text-xs text-slate-400">
            ${isEn 
              ? 'Browse previous automatic snapshots and restore your document to any point in time.' 
              : 'Navegue pelos pontos de restauração gravados durante a sessão e recupere versões anteriores do trabalho.'}
          </p>
  ...
  showDeleteConfirmModal(docId, docTitle) {
    const container = document.getElementById('modal-container');
    const isEn = window.AURA && window.AURA.currentLang === 'en';

    container.innerHTML = `
      <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
        <div class="glass-panel w-full max-w-sm rounded-2xl p-5 sm:p-6 border border-rose-900/50 shadow-2xl flex flex-col gap-4">
          <div class="flex items-center gap-3 text-rose-400">
            <div class="p-2.5 rounded-xl bg-rose-950/80 border border-rose-800">
              <i data-lucide="trash-2" class="w-6 h-6"></i>
            </div>
            <div>
              <h3 class="text-base font-bold text-white">${isEn ? 'Delete Academic Work?' : 'Excluir Trabalho Acadêmico?'}</h3>
              <p class="text-[11px] text-slate-400">${isEn ? 'This action cannot be undone.' : 'Esta ação não poderá ser desfeita.'}</p>
            </div>
          </div>

          <p class="text-xs text-slate-300 bg-slate-900/80 p-3 rounded-xl border border-slate-800 font-medium">
    ...
    container.innerHTML = `
      <div class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
        <div class="glass-panel w-full max-w-md rounded-2xl p-5 sm:p-6 border border-slate-700 shadow-2xl flex flex-col gap-4">
          <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 class="text-base font-bold text-white flex items-center gap-2">
              <i data-lucide="replace" class="w-5 h-5 text-amber-400"></i> ${isEn ? 'Find & Replace' : 'Localizar e Substituir Avançado'}
            </h3>
            <button onclick="AURA.closeModal()" class="text-slate-400 hover:text-white"><i data-lucide="x" class="w-5 h-5"></i></button>
          </div>
          ...
          <div class="flex flex-col gap-3 text-xs">
            <div>
              <div class="flex items-center justify-between mb-1">
                <label class="text-slate-300 font-semibold">Localizar termo:</label>
                <div class="flex items-center gap-1">
                  <button onclick="AURA.findPrevMatch()" title="Anterior (Shift+F3)" class="p-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300">
                    <i data-lucide="chevron-up" class="w-3 h-3"></i>
                  </button>
                  <button onclick="AURA.findNextMatch()" title="Próximo (F3)" class="p-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300">
              ...
              <input 
                type="text" 
                id="modal-find-input" 
                value="${currentFindTerm}" 
                placeholder="Termo a ser localizado..." 
                oninput="AURA.onFindInputChange(this.value)" 
                class="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-amber-400 shadow-inner"
              />
            </div>

            <div>
              <label class="text-slate-300 font-semibold block mb-1">Substituir por:</label>
              <input 
                type="text" 
            ...
            <div class="flex items-center gap-4 py-1 text-slate-400">
              <label class="flex items-center gap-1.5 cursor-pointer">
                <input type="checkbox" id="modal-find-opt-case" class="rounded bg-slate-900 border-slate-700 text-amber-500 focus:ring-0">
                <span>Diferenciar Maiúsculas/Minúsculas</span>
              </label>
              <label class="flex items-center gap-1.5 cursor-pointer">
                <input type="checkbox" id="modal-find-opt-word" class="rounded bg-slate-900 border-slate-700 text-amber-500 focus:ring-0">
                <span>Palavra Inteira</span>
              </label>
            </div>

            <div id="modal-find-replace-results" class="text-[11px] font-mono text-amber-300 p-2 rounded-lg bg-slate-900/60 border border-slate-800/80 min-h-[30px] flex items-center">
              Dica: Use "Substituir 1" para navegar ou "Substituir Tudo" para lote.
            </div>
          </div>

          <div class="flex items-center justify-between pt-3 border-t border-slate-800">
            <button onclick="AURA.closeModal()" class="px-3.5 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold">
              ${isEn ? 'Close' : 'Fechar'}
            </button>
            <div class="flex items-center gap-2">
              <button onclick="AURA.execModalFindAndReplace(false)" class="px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition-all border border-slate-700">
                Substituir 1
              </button>
              <button onclick="AURA.execModalFindAndReplace(true)" class="px-4 py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold transition-all shadow-md shadow-amber-600/30">
                Substituir Tudo
              </button>
            </div>
          </div>
